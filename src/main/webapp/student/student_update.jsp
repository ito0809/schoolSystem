...
<form action="<%= ctx %>/studentdata/StudentUpdateServlet" method="post">
  ...
  <div><label>学籍番号</label><input type="text" name="student_number"
    value="<%= cd.getStudentNumber()==null? "": cd.getStudentNumber() %>"></div>
  <div><label>姓</label><input type="text" name="last_name"
    value="<%= cd.getLastName()==null? "": cd.getLastName() %>" required></div>
  <div><label>名</label><input type="text" name="first_name"
    value="<%= cd.getFirstName()==null? "": cd.getFirstName() %>" required></div>
  <div><label>セイ</label><input type="text" name="last_name_kana"
    value="<%= cd.getLastNameKana()==null? "": cd.getLastNameKana() %>"></div>
  <div><label>メイ</label><input type="text" name="first_name_kana"
    value="<%= cd.getFirstNameKana()==null? "": cd.getFirstNameKana() %>"></div>
  <div><label>生年月日</label><input type="date" name="birth_date"
    value="<%= cd.getBirthDate()==null? "": cd.getBirthDate() %>"></div>
  ...
  <div><label>郵便番号</label><input type="text" name="postal_code"
    value="<%= cd.getPostalCode()==null? "": cd.getPostalCode() %>"></div>
  <div><label>都道府県</label><input type="text" name="prefecture"
    value="<%= cd.getPrefecture()==null? "": cd.getPrefecture() %>"></div>
  <div><label>市区町村</label><input type="text" name="city"
    value="<%= cd.getCity()==null? "": cd.getCity() %>"></div>
  <div><label>番地等</label><input type="text" name="address_line"
    value="<%= cd.getAddressLine()==null? "": cd.getAddressLine() %>"></div>
  <div><label>TEL</label><input type="text" name="tel"
    value="<%= cd.getTel()==null? "": cd.getTel() %>"></div>

  <!-- 学校はID入力のまま -->
  <div><label>学校ID</label><input type="number" name="school_id"
    value="<%= cd.getSchoolId() %>" required></div>

  <div><label>入学日</label><input type="date" name="enrollment_date"
    value="<%= cd.getEnrollmentDate()==null? "": cd.getEnrollmentDate() %>"></div>
  <div><label>卒業日</label><input type="date" name="graduation_date"
    value="<%= cd.getGraduationDate()==null? "": cd.getGraduationDate() %>"></div>
