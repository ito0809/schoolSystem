...
<!DOCTYPE html><html><head><title>コース削除</title></head><body>
<h1>コース削除</h1>
<%
...
<ul>
  <li>ID: <%=cd.getCourseId()%></li>
  <li>コース名: <%=cd.getCourseName()%></li>
  <li>学校ID: <%=cd.getSchoolId()%></li>
</ul>
<form action="<%=ctx%>/coursedata/CourseDeleteServlet" method="post">
  <input type="hidden" name="id" value="<%=cd.getCourseId()%>">
