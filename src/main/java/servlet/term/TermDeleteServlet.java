...
import dao.term.TermDeleteDao;
import dao.term.TermListDao;
import model.term.TermData;

@WebServlet("/termdata/TermDeleteServlet")
public class TermDeleteServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
      ...
      req.getRequestDispatcher("/term/term_delete.jsp").forward(req, resp);
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
      int id = Integer.parseInt(idStr);
      boolean ok = new TermDeleteDao().delete(id);
      resp.sendRedirect(req.getContextPath()+"/terms?deleted="+(ok? id : "0"));
